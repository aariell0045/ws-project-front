...
function ContactsList() {
	...
	async function editContactProfile(contactId, newContact) {
		const response = await fetch(`${process.env.React_App_HEROKU_SERVER_URL}/edit-group`, {
			method: "PUT",
	...
	async function addContactToGroup(newContact) {
		const response = await fetch(`${process.env.React_App_HEROKU_SERVER_URL}/contact`, {
			method: "PUT",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
				userId: userId,
				...
				<div className="contacts-page-main-header-right-side">
					...
					<div className="contacts-page-main-header-title-container">
						<p className="contacts-page-main-header-title">
							<span>{group.groupName} </span>- {group.contacts.length} משתתפים
						</p>
					</div>
				</div>
			...
			<div className="contacts-page-contacts-list">
				{state.searchContacts &&
					contactsList.map((contact, index) => {
						const isContactName = contact.contactProfile.contactFirstName.includes(state.searchContacts);
						const isContactPhoneNumber = contact.contactProfile.contactLastName.includes(state.searchContacts);
