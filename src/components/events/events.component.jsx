...
function Events({ currentDayInTheWeek, currentDate, event, setAllEvents, allEvents }) {
	...
	function mapEvents(event, index) {
		return (
			<div
				onClick={() => {
					setEventToDisplay({ headerText: event.eventName, contentText: event.eventContent });
				}}
				key={index}
			...
			>
				...
				<div
					onClick={async (e) => {
						...
						const updateEvents = events.filter((e) => Handler.filterEvents(e, event));
	...
	const showEvent = eventToDisplay.headerText !== undefined || eventToDisplay.contentText !== undefined;
	...
	return (
		<div className="events-container">
			...
			{popUpWindow && (
				<div className="add-new-event-window">
					<div className="add-new-event-sub-header">
						...
						<span className="context-header">{"אירוע חדש"}</span>
					</div>
					<header className="add-new-event-header">
						<input
							onChange={({ target: { value } }) => Handler.handleTextRenderer(value, setEventName)}
							value={eventName}
					...
					<div className="add-new-event-button-container">
						<button
							onClick={async () => {
								...
								setAllEvents(updateAllEvents);
								Handler.clearState("", setEventContent);
								Handler.clearState("", setEventName);
								close();
							}}
			...
			{showEvent && <Event setEventToDisplay={setEventToDisplay} {...eventToDisplay} />}
		</div>
	);
}
